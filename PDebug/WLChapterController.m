...
- (void)viewDidLoad
{
    ...
    _imageView.frame = CGRectMake(16, title.bottom + 8, imageWidth, image.size.height * imageWidth / MAX(image.size.width, 0.00001));
    _imageView.userInteractionEnabled = YES;
    UITapGestureRecognizer *tap = [[UITapGestureRecognizer alloc] initWithTarget:self action:@selector(presentImage)];
    [_imageView addGestureRecognizer:tap];
    [_contentView addSubview:_imageView];
    
    for (int i = 0; i < _chapter.steps.count; i ++) {
        WLStep *step = _chapter.steps[i];
        UIButton *btn = [[UIButton alloc] initWithFrame:CGRectMake(16, _imageView.bottom + 8 + 44 * i, self.view.width - 16 * 2, 44)];
    ...
    UILabel *label = [[UILabel alloc] initWithFrame:CGRectMake(16, _imageView.bottom + 8 * 2 + 44 * _chapter.steps.count, self.view.frame.size.width - 16 * 2, 2000)];
...
- (void)presentImage
{
    WLImageViewController *vc = [[WLImageViewController alloc] initWithImage:_imageView.image];
